...
def get_data_loaders(dataset_path, batch_size, debug=False):
    ...
    dataloaders['test'] =  DataLoader(MSMarco(dataset_path, 'dev' , debug=debug),
        batch_size=batch_size, collate_fn=collate_fn_padd)
...
def get_data_loaders_online(dataset_path, batch_size, debug=False):
    ...
    dataloaders['test'] = DataLoader(MSMarcoInference(f'{dataset_path}/queries.dev.tsv.p',f'{dataset_path}/queries.dev.tsv.ids.p',  debug=debug),
    batch_size=batch_size, collate_fn=collate_fn_padd_ids)
    dataloaders['val'] =  DataLoader(MSMarcoInference(f'{dataset_path}/queries.eval.tsv.p', f'{dataset_path}/queries.eval.tsv.ids.p', debug=debug),
        batch_size=batch_size, collate_fn=collate_fn_padd_ids)
    return dataloaders
